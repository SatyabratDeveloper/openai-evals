...
class PromptTest(evals.Eval):
    ...
    def eval_sample(self, test_sample, rng: random.Random):
        ...
        # sampled = result.get_completions()[0]["model"]["output"]["similarity"]

        logging.info(f"result.get_completions()***: {result.get_completions()}")

        logging.info(f"sampled: {sampled}")
        logging.info(f"expected: {test_sample['ideal']}")

        # Convert the JSON string to a Python dictionary
        sampled_dict = json.loads(sampled.replace("'", "\""))
        expected_dict = test_sample["ideal"]

        sampled_lowercase_dict = {key.lower(): value.lower() if isinstance(value, str) else value for key, value in sampled_dict.items()}
        expected_lowercase_dict = {key.lower(): value.lower() if isinstance(value, str) else value for key, value in expected_dict.items()}
